...
from topwords import top_words
...
def search():
    try:
        ...
        text_box.delete('1.0',END)
        text_box.configure(fg="blue",font=('',15))
        c=1
        for k,v in res:
            text_box.insert(END,f"\t{c}. {k} : {v}\n")
            c+=1
    except Exception:
        messagebox.showinfo("Invalid File","Currupt file or not supported")
...
logo.place(x=20,y=400)

lbl_title=Label(root,text='TOPWORDS',font=('algerian',24,'bold italic'),bg='light grey')
lbl_title.place(x=150,y=10)

lbl_dir=Label(root,text="File: ",font=('',14,'bold'),bg='light grey')
lbl_dir.place(x=70,y=150)

lbl_key=Label(root,text="Topwords: ",font=('',14,'bold'),bg='light grey')
